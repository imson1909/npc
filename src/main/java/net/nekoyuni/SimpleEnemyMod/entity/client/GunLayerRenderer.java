      ...
      if (!entity.isDeadOrDying()) {
         ...
         if (stack.getItem() instanceof AbstractGunItem gunItem) {
            ...
            if (this.getParentModel() instanceof HierarchicalModel<?> hierarchicalModel) {
               ...
               poseStack.pushPose();
               unit.render(poseStack, buffer.getBuffer(RenderType.entityCutoutNoCull(getTextureLocation(entity))), packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
               rightArm.render(poseStack, buffer.getBuffer(RenderType.entityCutoutNoCull(getTextureLocation(entity))), packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
               rightHandAnchor.render(poseStack, buffer.getBuffer(RenderType.entityCutoutNoCull(getTextureLocation(entity))), packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
               if (isMinigun) {
