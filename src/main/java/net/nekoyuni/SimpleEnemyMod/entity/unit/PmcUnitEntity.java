...
public class PmcUnitEntity extends AbstractUnit implements ICommandableMob {
   ...
   public void setupRoleGoals() {
      ...
      if (!(Boolean)FactionsConfig.US_UNITS_FRIENDLY.get()) {
         this.targetSelector.addGoal(2, new VerticalAwareTargetGoal(this, USunitEntity.class, true, vRange, fireFilter));
      }

      if (!(Boolean)FactionsConfig.RU_UNITS_FRIENDLY.get()) {
         this.targetSelector.addGoal(2, new VerticalAwareTargetGoal(this, RUunitEntity.class, true, vRange, fireFilter));
      }
      ...
      this.targetSelector.addGoal(2, new VerticalAwareTargetGoal(this, Monster.class, true, vRange, target -> {
         if (target.getClass() == this.getClass()) {
            return false;
         } else if ((Boolean)FactionsConfig.RU_UNITS_FRIENDLY.get() && target instanceof RUunitEntity) {
            return false;
         } else if ((Boolean)FactionsConfig.US_UNITS_FRIENDLY.get() && target instanceof USunitEntity) {
            return false;
         } else {
