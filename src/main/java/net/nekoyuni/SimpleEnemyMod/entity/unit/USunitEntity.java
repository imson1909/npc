...
public class USunitEntity extends AbstractUnit {
   ...
   private int variant = this.random.nextInt(3);
   ...
   public void setupRoleGoals() {
      ...
      this.targetSelector.addGoal(1, new HurtByTargetGoal(this, new Class[]{USunitEntity.class}));
      if (!(Boolean)FactionsConfig.US_UNITS_FRIENDLY.get()) {
      ...
      this.targetSelector.addGoal(2, new NearestAttackableTargetGoal(this, RUunitEntity.class, true));
      ...
      this.targetSelector.addGoal(4, new VerticalAwareTargetGoal(this, IronGolem.class, true, vRange));
      this.targetSelector.addGoal(2, new VerticalAwareTargetGoal(this, Monster.class, true, vRange, target -> {
         if (target.getClass() == this.getClass()) {
            return false;
         } else if ((Boolean)FactionsConfig.US_UNITS_FRIENDLY.get() && target instanceof PmcUnitEntity) {
            return false;
         } else {
   ...
   protected SoundEvent getCustomHurtSound() {
      return (SoundEvent)ModSounds.SOUND_US_UNIT_HURT.get();
   }

   @Override
   protected SoundEvent getCustomDeathSound() {
      return (SoundEvent)ModSounds.SOUND_US_UNIT_DEATH.get();
   }

   @Override
   protected SoundEvent getCustomAlertSound() {
      return (SoundEvent)ModSounds.SOUND_US_UNIT_ALERT.get();
   }
